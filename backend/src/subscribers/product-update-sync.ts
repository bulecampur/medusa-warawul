...
export default async function productUpdateSyncHandler({
  event: { data },
  container,
  ...
  try {
    logger.info(`Product update sync event triggered for product ${productId}`);
    ...
    try {
      ...
      // Sync the updated product to Lexoffice
      await productSyncService.syncProduct(product);

      logger.info(
        `Successfully synced updated product ${productId} to Lexoffice`
      );
    } catch (retrieveError) {
      logger.error(
        `Failed to retrieve product ${productId} for update sync:`,
        retrieveError
      );
      ...
      try {
        ...
        logger.info(
          `Successfully synced updated product ${productId} to Lexoffice (basic data)`
        );
      } catch (basicError) {
        logger.error(
          `Failed to sync updated product ${productId} even with basic data:`,
          basicError
        );
        throw basicError;
      }
    ...
    logger.error(
      `Failed to sync updated product ${productId} to Lexoffice`,
      error
    );
